...
static const QByteArray itemHeader("JM"), plugyPageHeader("ST");
...
void MedianXLOfflineTools::saveCharacter()
{
 ...
 outputDataStream.device()->reset();*/
    ...
    for (QHash<Enums::ItemStorage::ItemStorageEnum, PlugyStashInfo>::const_iterator iter = _plugyStashesHash.constBegin(); iter != _plugyStashesHash.constEnd(); ++iter)
	{
		...
		if (std::find_if(items.begin(), items.end(), hasChanged) != items.end())
			continue;
        ...
        for (int page = 1; page <= info.lastPage; ++page)
        {
            ...
            plugyFileDataStream.writeRawData(plugyPageHeader.constData(), plugyPageHeader.size() + 1); // write '\0'
            plugyFileDataStream.writeRawData(itemHeader.constData(), itemHeader.size()); // do not write '\0'
            plugyFileDataStream << static_cast<quint16>(pageItems.size());
            writeItems(pageItems, plugyFileDataStream);
        }
	}
	...
	writeItems(characterItems, outputDataStream);
...
{
	QFile cubeFile(resourcesPath + "/data/cube.d2i");
	if (cubeFile.open(QIODevice::ReadOnly))
	{
		QByteArray cubeBytes = cubeFile.readAll();
		cubeFile.close();

		QDataStream ds(cubeBytes);
		ds.setByteOrder(QDataStream::LittleEndian);

		ItemInfo *cube = parseItem(ds, cubeBytes);
		cube->hasChanged = true;
		cube->location = Enums::ItemLocation::Stored;
		cube->row = cube->column = -1;

		if (!storeItemIn(Enums::ItemStorage::Inventory, 6, 10, cube) && !storeItemIn(Enums::ItemStorage::Stash, 10, 10, cube))
		{
		...
		if (cube->column)
		{
			...
			//cube->bitString.replace(cube->bitString.length() - (65-16) - 4, 4, QString::number(cube->column, 2));
		}
		if (cube->row)
		{
			ReverseBitWriter::replaceValueInBitString(cube->bitString, Enums::ItemOffsets::Rows, 3, cube->row);
			//cube->bitString.replace(cube->bitString.length() - (69-16) - 3, 3, QString::number(cube->row, 2));
		}
		if (cube->storage != Enums::ItemStorage::Inventory)
		{
			...
			//cube->bitString.replace(cube->bitString.length() - (73-16) - 3, 3, QString::number(cube->storage, 2));
		}
		...
		ERROR_BOX(tr("Error opening Cube file.\nReason: %1").arg(cubeFile.errorString()));
}
...
{
    QFile f(resourcesPath + "/data/exptable.txt");
...
bool MedianXLOfflineTools::processSaveFile(const QString &charPath)
{
 ...
 for (int i = 0; i < 3; ++i)
 {
  ...
  inputDataStream.skipRawData(14);
    ...
    int totalStats = 0;
#ifdef PARSE_ALL_CHARACTERS
    bool wasHackWarningShown = true;
#else
    ...
    {
        //qDebug() << "skill" << i << "=" << int(_saveFileContents.at(skillOffset + 2 + i));
    ...
    if (_saveFileContents.mid(charItemsOffset, 2) != itemHeader)
    {
    ...
    //	qDebug() << "char items total" << charItemsTotal;
    ...
    {
        editableCharInfo.items.character += parseItem(inputDataStream, _saveFileContents);
    }
	...
	for (QHash<Enums::ItemStorage::ItemStorageEnum, PlugyStashInfo>::iterator iter = _plugyStashesHash.begin(); iter != _plugyStashesHash.end(); ++iter)
	{
		...
		//if (iter.key() == Enums::ItemStorage::SharedStash && !sharedStashPathChanged || iter.key() == Enums::ItemStorage::HCStash && !hcStashPathChanged)
		//	continue;
...
bool MedianXLOfflineTools::processPlugyStash(QHash<Enums::ItemStorage::ItemStorageEnum, PlugyStashInfo>::iterator &iter, ItemsList *items)
{
    ...
    if (info.hasGold = (bytes.mid(headerSize + 1 + 4, 2) != plugyPageHeader))
    ...
    for (quint32 page = 1; page <= info.lastPage; ++page)
    {
        ...
        {
            ItemInfo *item = parseItem(inputDataStream, bytes);
...
ItemInfo *MedianXLOfflineTools::parseItem(QDataStream &inputDataStream, const QByteArray &bytes)
{
	ItemInfo *item = 0;
	bool ok = true;
	...
	do 
	{
		...
		if (item->isExtended)
		{
			...
			case Enums::ItemQuality::Honorific:
				...
				break;
			default:
				WARNING_BOX(tr("Item '%1' of unknown quality '%2' found!").arg(itemBase.name).arg(item->quality));
				break;
			}
...
QMap<int, ItemProperty> MedianXLOfflineTools::parseItemProperties(ReverseBitReader &bitReader, bool *ok)
{
...
void MedianXLOfflineTools::updateUI()
{
 ...
 ui.currentActSpinBox->setValue(_editableCharInfo.basicInfo.currentAct);*/
	...
	//bool hasCube = false;
	//foreach (ItemInfo *item, _editableCharInfo.items.character)
	//	if ((item->storage == Enums::ItemStorage::Inventory || item->storage == Enums::ItemStorage::Stash) && item->itemType == "box")
	//	{
	//		hasCube = true;
	//		break;
	//	}
	ui.actionGiveCube->setDisabled(std::find_if(_editableCharInfo.items.character.begin(), _editableCharInfo.items.character.end(), isCube) != _editableCharInfo.items.character.end());
...
void MedianXLOfflineTools::writeItems(const ItemsList &items, QDataStream &ds)
{
	foreach (ItemInfo *item, items)
	{
...
void MedianXLOfflineTools::clearItems(bool sharedStashPathChanged /*= true*/, bool hcStashPathChanged /*= true*/)
{
	...
	while (itemIterator.hasNext())
	{
		...
		item->socketablesInfo.clear();
		delete item;
		itemIterator.remove();
	}
}

bool MedianXLOfflineTools::storeItemIn(Enums::ItemStorage::ItemStorageEnum storage, quint8 rows, quint8 cols, ItemInfo *item)
{
	ItemsList items = ItemsViewerDialog::itemsLocatedAt(storage);
...
bool MedianXLOfflineTools::canStoreItemAt(quint8 row, quint8 col, const QByteArray &itemType, const ItemsList &items) const
{
